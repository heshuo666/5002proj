...
class VariationalEncoder(nn.Module):
    def __init__(self, latent_dims, height, wide):
        ...
        self.conv1 = nn.Conv1d(1, 8, (4, wide))
        self.conv2 = nn.Conv1d(8, 16, (4, 1))
        self.batch2 = nn.BatchNorm2d(16)
        self.conv3 = nn.Conv1d(16, 32, (4, 1))
        self.linear1 = nn.Linear(32 * (height - 9) * 1, 128)
...
class Decoder(nn.Module):
    def __init__(self, latent_dims, height, wide):
        ...
        self.decoder_lin = nn.Sequential(
            ...
            nn.ReLU(True),
            nn.Linear(128, 32 * (wide - 3) * height),
            nn.ReLU(True)
        )
        self.unflatten = nn.Unflatten(dim=1, unflattened_size=(32, wide - 3, int(height, )))
        self.decoder_conv = nn.Sequential(
            nn.ConvTranspose1d(32, 16, (2, 1)),
            nn.BatchNorm2d(16),
            nn.ReLU(True),
            nn.ConvTranspose2d(16, 8, (2, 1)),
            nn.ReLU(True),
            nn.ConvTranspose1d(8, 1, (2, 1))
        )
    def forward(self, x):
...
def train_epoch(vae, device, train_window_samples, optimizer, epoch):
    vae.train()
    train_loss = 0.0
    flag = 0
    for x in train_window_samples:
        ...
        if flag == 0:
            ...
            plt.plot(x[0][0][0].cpu(), color='green', label="x")
            plt.plot(x_hat[0][0][0].cpu().detach().numpy(), color='red', label="x_hat")
            plt.title('1' + ' train sample ' + f'epoch {epoch}')
            plt.legend()
            plt.savefig(
                f'data-sets/KDD-Cup/VAE/{epoch}_train_sample.png',
...
def _test_epoch(vae, device, test_window_samples, epoch):
    vae.eval()
    test_loss = 0.0
    with torch.no_grad():
        flag = 0
        for x in test_window_samples:
            ...
            if flag == 0:
                ...
                plt.rcParams['figure.figsize'] = (20, 16)
                plt.plot(x[0][0][0].cpu(), color='green', label="x")
                plt.plot(x_hat[0][0][0].cpu(), color='red', label="x_hat")
                plt.title('1' + ' test sample ' + f'epoch {epoch}')
                plt.legend()
                plt.savefig(
                    f'data-sets/KDD-Cup/VAE/{epoch}_test_sample.png',
                    dpi=1000, bbox_inches='tight')
                plt.close()
...
def main():
    ...
    for select in range(1, 2):
        ...
        for epoch in range(num_epochs):
            train_loss = train_epoch(vae, device, train_window_samples, optimizer, epoch)
            test_loss = _test_epoch(vae, device, test_window_samples, epoch)
